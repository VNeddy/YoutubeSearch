        ...
        if form.is_valid():  # 进行数据校验
            ...
            with open("key", "r") as f:
                for line in f.readlines():
                    developer_key_list.append(line.strip('\n'))
            is_succeed = False
            link = "查询失败，请重试！！！"
...
def youtube_search(developer_key, proxy_host, proxy_port, key_word, order, scope_start, scope_end, max_result):
    ...
    item_list = ["Youtube Id", "主页Link", "粉丝数", "最低views", "最高views", "平均views(去掉最低最高)", "互动率",
                 "平均likes", "平均comments", "国家", "简介", "注册时间", "1个月内上传视频数", "发布1天",
                 "1-7天", "1-2周", "2-3周", "3周-1月",
                 "1-2个月", "2-3个月", "3-6个月"]
    ...
    split_channel_list = split_list(channels, x)
    for channel_list in split_channel_list:
        ...
        for channels_result in channels_response.get('items', []):
            try:
                ...
                if len(video_ids) != 0:
                    if len(video_ids) > 3:
                        avg_view_count = round((view_count - lowest_views - highest_views) / (len(video_ids) - 2))
                    else:
                ...
                if channel_subscriber_count != '' and int(channel_subscriber_count) != 0:
                    engagement_rate = round((avg_view_count / int(channel_subscriber_count)) * 100, 2)
                else:
                ...
                li_data.append(str(engagement_rate) + "%")
